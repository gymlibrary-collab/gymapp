...
create policy "sessions_read" on sessions
  for select using (
    ...
    (get_user_role() = 'manager' and gym_id = get_manager_gym_id())
    -- Trainer:
    --   My Sessions view  → own sessions (all statuses, all assigned gyms)
    --   Full Gym Schedule → all sessions at any gym they are assigned to
    --   RLS permits both; the app query layer filters status/trainer_id
    or (get_user_role() = 'trainer' and (
      -- Own sessions (all statuses)
      trainer_id = auth.uid()
      -- Any gym the trainer is assigned to (for gym schedule view)
      or gym_id in (
