...
### Commission payout adjustments
If a commission payout is found to have errors after generation, do not delete it. The difference is handled as a manual adjustment entry in the following month's payout. This avoids the complexity of resetting commission_paid flags on source records (sessions, packages, gym_memberships) which would risk double-payment.
