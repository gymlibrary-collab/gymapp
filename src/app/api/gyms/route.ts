...
export async function GET(req: NextRequest) {
  ...
  if (staffId) {
    // Self-query: always allowed — user querying their own gym assignments
    if (staffId !== user.id) {
      // Third-party query: verify access rights
      ...
      if (requester.role === 'manager') {
        const { data: check } = await adminClient
          ...
          .eq('trainer_id', staffId).eq('gym_id', requester.manager_gym_id).maybeSingle()
