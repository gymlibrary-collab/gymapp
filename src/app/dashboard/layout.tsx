...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const sectionMap: Record<string, string[]> = {
      'HR & Payroll': ['/dashboard/payroll', '/dashboard/payroll/commission', '/dashboard/payroll/annual', '/dashboard/payroll/cpf'],
      'Gym Operations': ['/dashboard/membership/sales', '/dashboard/hr/staff', '/dashboard/hr/leave', '/dashboard/membership/types', '/dashboard/pt/packages', '/dashboard/config/public-holidays'],
      'Configuration': ['/dashboard/config/gyms', '/dashboard/config/commission', '/dashboard/config/leave-policy', '/dashboard/config/escalation', '/dashboard/config/whatsapp-templates', '/dashboard/config/whatsapp-notifications'],
    }
      ...
      <nav className="flex-1 p-3 space-y-0.5 overflow-y-auto">
        {(() => {
          ...
          return nav.map((item, i) => {
            if (item.header) {
              if (item.collapsible) {
                const isCollapsed = collapsedSections[item.label] ?? true
              ...
              return <p key={i} className="text-xs font-semibold text-gray-400 uppercase tracking-wider px-3 pt-3 pb-1">{item.label}</p>
            }
