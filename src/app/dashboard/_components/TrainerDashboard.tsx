...
export default function TrainerDashboard({ user, isActingAsTrainer = false }: TrainerDashboardProps) {
  ...
  const [stats, setStats] = useState<any>({ members: 0, packages: 0, sessions: 0, commission: 0, ptSessionTotal: 0, ptSignupTotal: 0, membershipTotal: 0, totalCommissionPayout: 0 })
  ...
  const load = async (silent = false) => {
    ...
    if (!silent) logActivity('page_view', 'Dashboard', 'Trainer dashboard loaded')
      ...
      // ── Step 1: trainer gym assignments (gymIds needed for schedule filter) ──
      ...
      // ── Step 2: all remaining reads in parallel ────────────
      ...
      const [
        ...
        // Upcoming sessions
        fetchUpcomingSessions(supabase, { trainerId: user.id, todayEnd }),
        // Gym schedule (uses gymIds from Step 1)
        schedQ,
        // Stats — active packages for member count
        supabase.from('packages').select('member_id').eq('trainer_id', user.id).eq('status', 'active'),
        // Stats — package count
        supabase.from('packages').select('id', { count: 'exact', head: true }).eq('trainer_id', user.id).eq('status', 'active'),
        // Stats — completed sessions this month
        supabase.from('sessions').select('id').eq('trainer_id', user.id).eq('status', 'completed').gte('marked_complete_at', monthStart),
        // Commission stats from commission_items
        fetchCommissionStats(supabase, { userId: user.id, periodMonth: curMonth, periodYear: curYear, isTrainer: true }),
        // Package alerts
        fetchLowSessionPackages(supabase, { trainerId: user.id, limit: 10 }),
        fetchExpiringPackages(supabase, { trainerId: user.id, withinDays: 7, limit: 10 }),
