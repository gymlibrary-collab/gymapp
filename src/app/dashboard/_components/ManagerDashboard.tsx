...
export default function ManagerDashboard({ user }: ManagerDashboardProps) {
  ...
  const load = async (silent = false) => {
      ...
      // ══════════════════════════════════════════════════════
      // PHASE 1 — All critical reads fired in parallel
      // None depend on each other's result
      // ══════════════════════════════════════════════════════
      ...
      const [
        ...
        supabase.from('gym_memberships').select('price_sgd, commission_sgd').eq('gym_id', gymId).eq('sale_status', 'confirmed').gte('created_at', monthStart),
        // Pending banner counts
        supabase.from('gym_memberships').select('id', { count: 'exact', head: true }).eq('sold_by_user_id', user.id).eq('sale_status', 'pending'),
        supabase.from('membership_cancellation_requests').select('id', { count: 'exact', head: true }).eq('gym_id', gymId).eq('status', 'pending'),
        // Dispute notifications
        supabase.from('manager_dispute_notif').select('*').eq('manager_id', user.id).is('seen_at', null).order('created_at', { ascending: false }),
        // Pending confirmations (helper functions)
        fetchPendingMemberships(supabase, gymId),
        fetchPendingSessionConfirmations(supabase, gymId),
      ...
      setStats({ members: memberCount || 0, packages: pkgCount || 0, sessions: sessData?.length || 0, commission: 0, membershipRevenue, membershipSalesCount: memSalesData?.length || 0, totalCommissionPayout: 0 })
      setPendingMemSales(pendingCount || 0)
      setPendingCancellations(cancelCount || 0)
      setDisputeNotifs(dispNotifs || [])
      setPendingMemberships(pendingMems)
      setPendingSessions(pendingSess)
      ...
      // ══════════════════════════════════════════════════════
      // PHASE 2 — Non-critical reads fired in parallel
      ...
      const [
        ...
        fetchLowSessionPackages(supabase, { gymId, limit: 10 }),
        fetchExpiringPackages(supabase, { gymId, withinDays: 7, limit: 10 }),
        fetchExpiringMemberships(supabase, gymId, { withinDays: 30, limit: 20 }),
        fetchAtRiskMembers(supabase, gymId),
        fetchPendingLeave(supabase, gymId, user.id),
      ])
      ...
      setExpiringPackages(expiringPkgs)
