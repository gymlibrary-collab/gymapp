  ...
  useEffect(() => {
    const load = async () => {
      ...
      const { data: gymData } = await supabase
        .from('gyms').select('*').eq('is_active', true)
      setGyms(gymData || [])
      ...
      <form onSubmit={handleSubmit} className="card p-4 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <input className="input" type="date" required min={today}
              value={form.scheduled_date} onChange={set('scheduled_date')} />
          </div>
          <div>
            <label className="label">Time *</label>
            <input className="input" type="time" required
              value={form.scheduled_time} onChange={set('scheduled_time')} />
          </div>
        </div>
